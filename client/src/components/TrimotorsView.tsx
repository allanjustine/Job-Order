...
const TrimotorsPreviewJobOrder = ({ data, jobOrderId }: TrimotorsPreviewJobOrderProps) => {
    ...
    return (
      <div className="flex justify-center items-center p-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm">Loading trimotors job order data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="text-center p-8">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
          <p>{error}</p>
          <button onClick={() => window.location.reload()} className="mt-2 bg-red-500 text-white px-4 py-1 rounded text-sm">Retry</button>
        </div>
      </div>
    );
  }
  ...
  const renderStatusCell = (dataKey: string, statusType: string) => {
    const item = displayData.diagnosis?.[dataKey];
    ...
    <>
      {jobOrders.length > 0 && !data && (
        <div className="mb-4 p-4 bg-gray-100 rounded-lg shadow-sm print:hidden">
          <label className="font-bold mr-3 text-sm">Select Trimotors Job Order:</label>
          <select value={selectedJobId} onChange={(e) => handleJobOrderChange(e.target.value)} className="border border-gray-300 p-2 rounded-md text-sm">
            {jobOrders.map((job) => (
      ...
      <div className="p-1 font-sans bg-white text-black leading-tight border-2 border-black" style={{ fontSize: "8pt", maxWidth: "210mm", minHeight: "158mm", margin: "0", lineHeight: "0.10" }}>
        {/* Header */}
        ...
        <div className="mb-2 grid grid-cols-2 gap-x-4 gap-y-1" style={{ fontSize: "8pt", lineHeight: "0.8" }}>
          ...
          <div className="flex">
            ...
            <span className="border-b border-black flex-1">{displayData.purchase_date || ""}</span>
          </div>
          <div className="flex">
            <span className="font-bold w-32">Contact Number:</span>
            <span className="border-b border-black flex-1">{displayData.customer?.contact || ""}</span>
          </div>
          <div className="flex">
          ...
          <div className="border border-black p-0.5">
            ...
            <div className="mb-1">
              <div className="flex items-center justify-center overflow-hidden" style={{ height: "145px" }}>
                <img src="/trimotors.png" alt="Trimotors Unit Diagram" className="w-full h-full object-cover" />
              </div>
        ...
        <div className="mb-1 text-xs" style={{ fontSize: "8pt", lineHeight: "0.8" }}>
          ...
          <div className="flex">
            ...
            <span className="border-b border-black flex-1">{displayData.generalRemarks || displayData.general_remarks || ""}</span>
          </div>
        </div>
        ...
        <div className="mt-0.5 grid grid-cols-3 gap-2 text-xs" style={{ fontSize: "8pt", lineHeight: "0.8" }}>
          <div className="text-center p-0.5">
            ...
            <p className="underline">{displayData.serviceAdvisor || displayData.service_advisor || ""}</p>
          ...
          <div className="text-center p-0.5">
            ...
            <p className="underline">{displayData.branchManager || displayData.branch_manager || ""}</p>
            <p className="text-xs text-gray-600" style={{ fontSize: "7pt" }}>(Signature Over Printed Name)</p>
          ...
          <div className="text-center p-0.5">
            ...
            <p className="underline">{displayData.customer?.name || displayData.customerName || ""}</p>
            <p className="text-xs text-gray-600" style={{ fontSize: "7pt" }}>(Signature Over Printed Name)</p>
            <p className="text-xxs">Customer</p>
          </div>
        </div>

        <p className="mt-2 text-center" style={{ fontSize: "6pt" }}>Printed on: {format(new Date(), "MMMM dd, yyyy hh:mm a")}</p>
      </div>
    </>
  );
};

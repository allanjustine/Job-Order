...
const MotorsPrintJobOrder = ({ data }: PrintJobOrderProps) => {
  ...
  const getCouponName = (couponId: string | undefined): string => {
    ...
    return (
      coupons.find((c) => c.name === couponId)?.name || `Coupon ${couponId}`
    );
  };
  ...
  const isPartSelected = (partKey: string): boolean => {
    return !!(
      data.partsReplacement?.[partKey as keyof PartsReplacement] &&
  ...
  const getPartsOthersItems = (): PartsOthersItem[] => {
    if (
      data.partsReplacement.partsOthers &&
      data.partsReplacement.partsOthersItems
    ) {
  ...
  const getAllSelectedJobs = () => {
    ...
    const jobOthersMarkers = jobOthersItems.map(
      (_, index) => `job_others_${index}`,
    );
  ...
  const getAllSelectedParts = () => {
    ...
    const partsOthersMarkers = partsOthersItems.map(
      (_, index) => `parts_others_${index}`,
    );

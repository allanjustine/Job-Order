...
const ViewJobOrder = ({ data, isReprint = false }: PreviewJobOrderProps) => {
  ...
  `}</style>
      ...
      {Array.from({ length: isReprint ? 2 : 1 }).map((_, index) => (
        <div
          className={`jo-rotate-wrapper ${isReprint ? `mt-3 ml-3 ${index === 1 ? "border-b border-black border-dashed" : ""}` : ""}`}
          key={index}
        >
          <div
          ...
          <TriMotorsImage data={data} />
            ...
            >
              ...
              <div className="text-center p-0.5">
                ...
                <p style={{ fontSize: isReprint ? "7pt" : "9pt" }}>Customer</p>
              </div>
            </div>
